 ...
 */
@Stateless
public class ViewKeywordTotalOptInsManager extends AbstractManager<ViewKeywordTotalOptIns> implements ViewKeywordTotalOptInsManagerLocal {
    ...
    public ViewKeywordTotalOptInsManager() {
        super(ViewKeywordTotalOptIns.class);

    }

    @Override
    public List<ViewKeywordTotalOptIns> getAllByBrand(Brand brand) {
        String queryString = "SELECT ks FROM ViewKeywordTotalOptIns ks WHERE ks.company.brand = :brand";
        Query q = em.createQuery(queryString);
        q.setParameter("brand", brand);
        List<ViewKeywordTotalOptIns> results = (List<ViewKeywordTotalOptIns>) q.getResultList();
        if (results.size() > 0) {
            return results;
        }
        return null; // none
    ...
    public List<ViewKeywordTotalOptIns> getAllByCompany(Company company) {
        String queryString = "SELECT ks FROM ViewKeywordTotalOptIns ks WHERE ks.company = :company";
        Query q = em.createQuery(queryString);
    ...
    public List<ViewKeywordTotalOptIns> getAllByKeyword(Keyword keyword) {
        String queryString = "SELECT ks FROM ViewKeywordTotalOptIns ks WHERE ks.keyword = :keyword";
        Query q = em.createQuery(queryString);
    ...
    public long getTotalByKeywordAndDate(Keyword keyword, Date startDate, Date endDate) {
        String queryString = "SELECT sum(ks.totalOptIns) FROM ViewKeywordTotalOptIns ks WHERE ks.keyword = :keyword";
    ...
    public Map<Keyword, Long> getTotalPerKeywordByCompanyAndDate(Company company, Date startDate, Date endDate) {
        String queryString = "SELECT SUM(total_opt_ins), keyword_id FROM view_keyword_total_opt_ins WHERE company_id = ?1";
        if (startDate != null && endDate != null) {
