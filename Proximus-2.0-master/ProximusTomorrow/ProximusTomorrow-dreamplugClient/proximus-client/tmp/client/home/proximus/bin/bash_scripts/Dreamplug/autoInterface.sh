...
function autoInterface()
{
	...
	for (( i=0; i<=9; i++ ))
	do
		/sbin/ifconfig "uap$i" up 2>/dev/null
	done
    ##List all interfaces starting with wlan NOT mon.wlan
    IFS=$'\n'
    interfaces=(`/sbin/ifconfig -a | egrep "^uap[0-9]" | awk '{print $1" "$5}' | sed 's/://g'`)
...
{
	EXISTING_BACKHAUL=""
    possible_interfaces="eth0 eth1 `autoInterface` ppp0"
    for interface in $possible_interfaces; do
    	VALID_IP=$(/sbin/ifconfig $interface 2>/dev/null | grep 'inet addr')
    	if [ "$VALID_IP" != "" ]; then
    	    if [ -f $EXTFILE ]; then
